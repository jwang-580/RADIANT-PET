...
def parse_response(decoded_text):
    ...
    """
    match_final = re.search(r"assistantfinal(.*)", decoded_text, re.DOTALL)
    match_analysis = re.search(r"assistantanalysis(.*)", decoded_text, re.DOTALL)
    if match_final:
        content_to_parse = match_final.group(1)
    elif match_analysis:
        content_to_parse = match_analysis.group(1)
    else:
        content_to_parse = decoded_text

    matches = list(re.finditer(r"(physiological_site|lesion_site)\s*:\s*(?:\[\s*([^\]]+)\s*\]|([^\n]+))", content_to_parse, re.IGNORECASE))
...
def process_file(file_path, model, tokenizer, canonical_sites, use_report=False, debug=False, output_dir=None):
    ...
    output_file = (
        ...
        if output_dir
        else file_path.replace(".json", "_predictions.json")
    )
    if os.path.exists(output_file):
        print(f"Skipping {file_path}: predictions already exist at {output_file}")
        return
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="unsloth/gpt-oss-20b-BF16", help="Path to the model.")
    ...
    # Create output directory based on model_path
    model_name = os.path.basename(args.model_path.rstrip('/'))
    data_dir_name = os.path.basename(args.data_dir.rstrip('/'))
    output_dir = f"{model_name}_{data_dir_name}_results"
