...
def generate_training_data(
    ...
    output_file="training_with_report_tp_data.jsonl",
    ...
    # Load lymphoma sites from site list
    try:
        with open(site_list_path, 'r') as f:
            site_data = json.load(f)
            lymphoma_sites = site_data.get("lymphoma_sites", [])
            if not lymphoma_sites:
                print("Warning: 'lymphoma_sites' not found or empty in site list file.")
                return
    ...
    for json_file_path in tqdm(json_files, desc="Processing files"):
        try:
            ...
            if 'lesions' not in gt_data:
                continue

            for lesion in gt_data['lesions']:
                # Check if this is a true lesion (skip if it's false)
                is_true_lesion = lesion.get("is_true_lesion", None)
                if is_true_lesion is False:
                    continue
                # Check volume
                ...
                radiology_report = ""
                if include_report:
                    radiology_report = gt_data.get('metadata', {}).get('pet_report', '')
                
                # Construct prompt based on whether report is included
                if include_report and radiology_report:
                    prompt = f"""
A PET/CT–derived description of a lymphoma lesion is provided below:
{lesion}

The radiology report for this case is provided below:
{radiology_report}

The lesion is known to belong to one of the following predefined nodal or extranodal sites:
{lymphoma_sites}

Using the anatomical coordinates, adjacent structures, organ relationships described, and the radiology report, determine which site most accurately corresponds to this lesion. 
Return only the name of the site, "lesion_site: <site_name>".
"""
                else:
                    prompt = f"""
A PET/CT–derived description of a lymphoma lesion is provided below:
...
Using the anatomical coordinates, adjacent structures, and organ relationships described, determine which site most accurately corresponds to this lesion. 
Return only the name of the site, "lesion_site: <site_name>".
"""
                try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate training data for lesion site classification using LLM"
    )
    ...
    parser.add_argument(
        ...
        help="Path to JSON file containing lymphoma site lists"
    )
    
    parser.add_argument(
        "--output-file",
        type=str,
        default="training_autopet_nnunet_tp_data.jsonl",
